...
description: na
search: na
title: Install%20the%20Windows%20PC%20client%20with%20Microsoft%20Intune
...
2. On the **Client Software Download** page, click **Download Client Software** and save the **Microsoft_Intune_Setup.zip** package containing the software to a secure location on your network.
   ...
   > [!NOTE]
   > The [!INC[wit_firstref](../Token/wit_firstref_md.md)] client software installation package contains information about your account. If unauthorized users gain access to the installation package, they can enroll computers to the account that is represented by its embedded certificate.
...
Users can self-enroll each of their computers through the [!INC[wit_firstref](../Token/wit_firstref_md.md)] company portal. Each enrolled computer is linked to the user account that was used to install the client software.
...
> - Each time a user self-enrolls a computer, it uses an [!INC[wit_nextref](../Token/wit_nextref_md.md)] license.
...
4. Click **Next** to start the [!INC[wit_firstref](../Token/wit_firstref_md.md)] Setup Wizard.
...
You can deploy the [!INC[wit_nextref](../Token/wit_nextref_md.md)] client software to computers as part of an operating system image by using the following example procedure as a basis:
...
3. Add the following command to **setupcomplete.cmd** to run the enrollment package with the /PrepareEnroll command-line argument:
   ...
   ```
   > [!TIP]
   > The **SetupComplete.cmd** script enables Windows Setup to make modifications to the system before a user logs on. The **/PrepareEnroll** command-line argument prepares a targeted computer to be automatically enrolled in [!INC[wit_nextref](../Token/wit_nextref_md.md)] after Windows Setup finishes.
...
2. Scroll down the list of computers to find managed computers that are communicating with [!INC[wit_nextref](../Token/wit_nextref_md.md)], or to search for a specific managed computer by typing the computer name, or any part of the name, in the **Search devices** box.
...
3. The **Computer Inventory Report** page opens in a new window that displays all computers that are successfully enrolled in [!INC[wit_nextref](../Token/wit_nextref_md.md)].
